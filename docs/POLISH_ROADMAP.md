...
- **02/07/2026 — Fase C concluída (microinterações de equipar / craftar / revelar).**
  - **Realce do afixo alterado após craft**: `src/ui/craftDiff.ts` (puro) diffa os afixos antes×depois por grupo+tipo, anotando **novo / alterado / removido**. `store.ts` guarda `lastCraft` (item antes, orbe usado, seq) e o limpa ao selecionar outro item. `ItemModList` mostra selo colorido + realce por mod; removidos aparecem riscados.
  - **Feedback de craftar**: brilho + "sheen" no `craft-preview` (`.just-crafted`/`.craft-flash`), pulso do orbe usado (`.orb-btn--used`) e **moeda decrementando** (`CraftCount` → "−1" subindo sobre o contador).
  - **Feedback de equipar**: `store.ts` guarda `lastEquip` (slot+seq); o slot do manequim faz **pulso de encaixe** (`.just-equipped`) com re-key por seq para reanimar ao reequipar.
  - **Transição estimativa→medido na PowerBar**: `PStat` ganhou prop `reveal`; ao virar "DPS (medido)" a caixa entra com glow/scale (`.pstat--reveal`), com `key` distinto p/ garantir o remount da animação.
  - **Tudo sob `prefers-reduced-motion`** (o reset global neutraliza as `animation`/`transition`). Motor (`src/game/engine.ts`) intocado; **só `store.ts` (estado React), pages, atoms, novo `craftDiff.ts` e CSS**. `typecheck` + 29 testes + `build` verdes; dev server serve 200 sem erros.
- **02/07/2026 — Fase D concluída (inventário, filtros e onboarding).**
  - **Ferramentas de inventário**: `src/ui/inventoryFilter.ts` (puro) filtra por **categoria de base** e **raridade**, faz **busca por afixo/nome/base** (case-insensitive) e **ordena** (recentes/raridade/nome/categoria, estável). `EquipmentPage.InventorySection` traz a barra (input + select + chips de categoria/raridade que só listam o que existe no baú), contador "N de M" e **estados vazios** escritos (baú vazio × filtro sem resultado, com "limpar filtros").
  - **Toasts unificados**: canal efêmero no `store.ts` (`toasts`, `pushToast`/`dismissToast`, tom good/warn/info/loot, teto de 4). `src/ui/Toasts.tsx` (`ToastHost`) montado no `App`, auto-dismiss por timer, `aria-live`, clique dispensa. Migrados os eventos transientes: **craft (sucesso/erro), "sem moedas", slot inválido, "DPS real descoberto"**. O `notice` inline ficou só para erros contextuais da árvore (alocação). Removido o texto solto de resultado na bancada (virou toast).
  - **Onboarding de 1ª sessão**: `src/ui/Onboarding.tsx` — modal de 4 passos (build 100% sua → montar no Equipamento → números descobertos → testar e ajustar), pulável, com Voltar/Próximo/Começar, pontos de progresso, Esc/clique-fora fecham. Flag `bw.onboarding.v1` no localStorage; botão **"?"** na top bar reabre a qualquer hora.
  - **Testes**: `src/ui/faseD.test.ts` cobre o filtro/ordenação/busca e o diff de afixos (12 casos). Motor (`src/game/`) **intocado** (o novo estado vive em `store.ts`, que é React, não o motor puro). `typecheck` + **41 testes** + `build` verdes; dev server serve 200 sem erros.
- **02/07/2026 — Fase E concluída (acessibilidade & mobile) + fundações F1 e F3.**
  - **Teclado** — nós da árvore (`TreePage`) viraram alvos focáveis (`role="button"`, `aria-pressed`, **roving tabindex** a partir da origem `s0`): **setas** movem o foco ao vizinho conectado mais alinhado à direção, **Enter/Espaço** aloca/reembolsa. Soquetes/palette da `SkillsPage` já eram `<button>` (operáveis) — ganharam **foco visível**. Adicionados **skip-link** ("pular para o conteúdo"), `aria-label` na nav e `aria-current` no item ativo.
  - **ARIA/leitor de tela** — minimapa da tentativa com `aria-label` que resume progresso e encontros alcançados; relatório vira `region` + `h3` com `aria-live`; count-up do DPS marcado `aria-hidden` com o valor final exposto no container; orbes ganham `aria-label` com nome/descrição/estoque.
  - **Mobile** — **pinch-zoom** e pan por toque no stage da árvore (`touchstart/move` com `touch-action: none`); **nav rolável** horizontal (em vez de quebrar); **alvos ≥44px** (nav/slots/orbes/botões/soquetes/chips) via media query. Painéis de detalhe já empilham (o detalhe da árvore é faixa fixa; detalhes de item usam o tooltip fixável clampeado) — bottom-sheet dedicado não foi necessário.
  - **Contraste** — verificação WCAG dos pares raridade/texto × superfícies. `--text-mut` (#5f6a78→#7c8899) e `--blood-hi` (#d8453b→#e35a4f) **clareados para passar AA (~5:1)**; raridades já passavam.
  - **F1 (tokens semânticos)** — bloco no `:root`: espaçamento (`--sp-1..6`), raios, sombras, durações/curvas de motion, tipografia, `--touch-min`, `--focus-ring`. Manifesto em **[THEME.md](./THEME.md)**. `.toast` refeito consumindo os tokens (critério de saída: usado por ≥1 componente).
  - **F3 (galeria de componentes)** — rota interna **`?dev=gallery`** (`src/pages/GalleryPage.tsx`, fora do fluxo de jogo) renderizando botões, chips, PowerBar (estimado×medido), ícones por raridade, orbes, tooltip, count-up e toasts. Base de regressão visual.
  - Motor (`src/game/`) **intocado**. `typecheck` + **41 testes** + `build` verdes; app e galeria servem 200.
- **Fase F (arte & áudio)** — trilha **assíncrona de produção de assets**, plugada pelo contrato F2 (`icon` em dados → SVG/arte). Não é entregável de código nesta rodada: os ids e componentes já aceitam a arte final sem mudança de código. Fica aberta até os assets existirem.

> **Polish Visual & UX — Fases A–E concluídas; fundações F1–F3 entregues; F2 já estava.** O que resta do roadmap é a **Fase F (arte/áudio)**, que depende de produção de assets. As próximas frentes de valor saem das **trilhas paralelas** (motor determinístico, online/persistência, conteúdo) — ver §8 e o HANDOFF.
