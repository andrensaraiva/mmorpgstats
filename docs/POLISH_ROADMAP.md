...
- **02/07/2026 — Fase C concluída (microinterações de equipar / craftar / revelar).**
  ...
  - **Próximo: Fase D** — filtros/ordenação/busca de afixo no inventário, onboarding de primeira sessão, toasts unificados. Fundações pendentes: **F1** (tokens semânticos) e **F3** (galeria de componentes).
