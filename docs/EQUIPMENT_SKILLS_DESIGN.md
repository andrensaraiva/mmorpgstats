...
1. **S1 — Modelo de item rico** (types+content+engine): qualidade, defesas, requisitos e novas famílias de afixo. Sem UI nova ainda.
2. **S2 — Tooltip/descrição PoE2** (Fase B do roadmap): mostra os novos campos com a hierarquia do print 3. Aqui entra também a comparação equipado × candidato.
3. **S3 — Habilidades por arma/classe:** `weaponType` nas armas, catálogo de skills com requisitos/tiers, `availableSkills`, e a tela de habilidades reorganizada por categoria (print 4).
