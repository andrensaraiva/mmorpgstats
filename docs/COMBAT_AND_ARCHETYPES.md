...
- ✅ **M2 — Armadura por golpe + evasão + ES + bloqueio como camadas reais de EHP. CONCLUÍDO (03/jul).** O EHP do herói (`aggregate`) deixou de ser `vida×(1+armour/(armour+1500))` e virou camadas: **pool = vida + ES**; mitigação física = armadura por tamanho de golpe (`A/(A+12×golpe)`, teto 90%) × **evasão** (valor esperado `ev/(ev+8×golpe)`, teto 95%) × bloqueio, contra um golpe de referência ∝ pool. No `simulateDungeon` (lado recebido), o físico passa pelas mesmas camadas e o **ES absorve antes da vida** (buffer). `Power` ganhou `evasion`/`energyShield`; StatKeys `evasion`/`incEvasion`/`energyShield`/`incEnergyShield`. Conteúdo: bases **Traje das Sombras** (evasão), **Vestes Arcanas** (ES), **Capuz de Seda** (híbrido); afixos flat/inc de evasão e ES; nós **Passo Leve**/**Dança das Sombras**; suporte **Salvaguarda**. UI: evasão/ES no dashboard (`PowerDetails`) e na comparação de item. **+4 testes** (62 no total). Determinismo mantido (evasão em valor esperado, sem RNG). Pendente do M2 pleno: dano recebido **elemental multi-camada** por onda (hoje o lado recebido divide por tipo mas usa magnitude do `diff`); refino fica com o M3/tuning.
- ✅ **M3 — Ailments/DoT. CONCLUÍDO (03/jul).** Golpes com `ailment` geram **dano ao longo do tempo** derivado do dano do golpe (do tipo do ailment) × `ailmentMult` × `(1+incDot)`, mitigado pela resistência do alvo no tick: **sangramento** (físico) e **queimadura** (fogo) refrescam (vale o stack mais forte, uptime pleno); **veneno** (caos) **empilha** (soma as aplicações → escala com a cadência de golpes). O DoT entra no `dps` e no breakdown por tipo/fonte (`dotDps`, fonte `'dot'` no `perSkill`). Conteúdo: skills **Dilacerar** (sangramento) e **Toque Pestilento** (veneno); afixo `inc DoT`; nós **Ferida Aberta**/**Praga Persistente**; suporte **Corrosão**. UI: DoT no diagnóstico do boneco + selo de ailment na skill. **+3 testes** (65 no total). Pendente (tuning): **ES não absorver DoT de sangramento/veneno no lado recebido** — sem efeito hoje porque o bestiário ainda não aplica DoT ao herói; chill/shock/freeze como escala/controle entram com o bestiário ativo.
- ✅ **M4 — Fontes múltiplas (minions, totens/balista). CONCLUÍDO (03/jul).** Skills com `source` (`minion`/`totem`) têm **orçamento de dano próprio** (`sourceDamage`/`sourceRate`/`sourceDamageType`) e contribuem **DPS contínuo, fora da rotação** ("posiciona e esquece"): `sourceDps = avg(dano) × taxa × (1+inc) × (1+more)`, mitigado pela defesa do alvo (armadura p/ físico, resistência p/ elemental). Some ao `dps` total e ao breakdown (`sourceDps`, fontes no `perSkill`). Conteúdo: **Guarda de Ossos** (minion físico) e **Totem Balista** (raio); afixos `inc minion`/`inc totem`; nós **Legião**/**Senhor das Hostes**; suportes **Comando Feroz**/**Ancoragem**. UI: fontes entram no pool da rotação (apesar de `damageMult 0`), com selo "Minion/Totém" e aparecem no diagnóstico do boneco. **+4 testes** (69 no total). Habilita summoner/totem.
- 🟢 **M5 — Execução por ticks. FEITO (via R1–R4 + M1–M4).** A simulação determinística no tempo que roda o combate real e gera o relatório existe e está completa para o protótipo: o **lado ofensivo** (`simulateRotation`: rotação + combo + DoT + fontes) e o **lado defensivo** (`simulateDungeon`: corrida limpar×morrer + CC + poções + EHP em camadas) alimentam o relatório do MVP. Refinos futuros são **tuning** (dano recebido por onda do bestiário, chill/shock/freeze ativos, ES vs. DoT recebido), não novo sistema. Tem **dois lados**:
